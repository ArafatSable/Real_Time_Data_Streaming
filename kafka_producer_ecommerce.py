...
class EcommerceDataProducer:
    ...
    def generate_data(self):
        try:
            while True:
                # Generate randomized data for all customers
                for _ in range(10):  # Adjust the number of records as needed
                    ...
                    satisfaction_level = (
                        'Satisfied' if average_rating > 4.0 else
                        'Neutral' if average_rating == 4.0 else
                        'Unsatisfied'
                    )

                    # Formulate the message as JSON
                    message = {
                    ...
                    print(f"Published JSON data: {json.dumps(message)}")
